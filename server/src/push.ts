...
import type {DiceRollResultWithID} from 'shared';
    ...
    switch (mutation.name) {
      case 'rollDice':
        await createDiceRoll(
          t,
          mutation.args as DiceRollResultWithID,
          nextVersion,
        );
        break;
      default:
...
async function createDiceRoll(
  ...
  console.log('creating dice roll', {id, result, timestamp, user_id});
  await t.none(
    `insert into dice_roll (
    id, user_id, result, timestamp, deleted, version) values
    ($1, $2, $3, $4, false, $5)`,
    [id, user_id, result, timestamp, version],
  );
}
